...
void load_program(char *program_filename) {
    ...
    // TODO
    //---------------------------------------------//
    // 1) text binary를 memory로 load 시키기
    for (i = 0; i < text_size / BYTES_PER_WORD; i++) {
        fgets(buffer, MIPS_REGS + 2, prog);
        load_inst_to_mem(buffer, i * BYTES_PER_WORD);
    }
    //--------------------------------------------//
    // 2) data binary를 memory로 load 시키기
    for (i = 0; i < data_size / BYTES_PER_WORD; i++) {
        fgets(buffer, MIPS_REGS + 2, prog);
        load_data_to_mem(buffer, i * BYTES_PER_WORD);
    }
    //-------------------------------------------//
